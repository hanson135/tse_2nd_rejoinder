...
\textbf{Response:}
\textcolor{red}{We thank you for the comment, which drove us to further understand what is the true originality and unique contribution of our study. We have given a lot of thoughts on it, and realised that we still needed to do a better job to highlight the study focus on large-scale agile methods, which is not reflected by how we organised and described the challenges and success factors in an adequate manner, in order to better distinguish our work from the previous ones, especially Dikert et al. [6]. To achieve this goal, we reviewed and revised the text in Section 5.3 and Section 5.4 carefully, to present all challenges and success factors from the method perspective, and re-organised them from the method perspective as well, as shown in the new Table 8 and Table 9. In doing so, we believe that we further distanced our work from the previous ones, to the point that a mapping of our challenges and success factors to theirs provides little value and insights. }
